...
impl std::fmt::Debug for BeatriceRC0 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BeatriceRC0")
...
impl BeatriceRC0 {
    ...
    pub fn load_model(&mut self, model_path: impl AsRef<Path>) -> Result<(), BeatriceError> {
        ...
        // speaker_embeddings
        {
            ...
            self.lib.codebooks.resize(
                n_speakers_plus_1 * (BEATRICE_20RC0_CODEBOOK_SIZE * BEATRICE_20RC0_PHONE_CHANNELS),
                0.0,
            );

            self.lib.additive_speaker_embeddings.resize(
                n_speakers_plus_1 * BEATRICE_WAVEFORM_GENERATOR_HIDDEN_CHANNELS,
                0.0,
            );

            self.lib
                .formant_shift_embeddings
                .resize(9 * BEATRICE_WAVEFORM_GENERATOR_HIDDEN_CHANNELS, 0.0);
    ...
    pub fn reset_context(&mut self) -> Result<(), BeatriceError> {
        unsafe {
            let phone_context = Beatrice20rc0_CreatePhoneContext1();
            let pitch_context = Beatrice20rc0_CreatePitchContext1();
            let waveform_context = Beatrice20rc0_CreateWaveformContext1();
            let embedding_context = Beatrice20rc0_CreateEmbeddingContext();
            ...
            let old_embedding_ctx = self.lib.embedding_context;

            self.lib.phone_context = phone_context;
            self.lib.pitch_context = pitch_context;
            self.lib.waveform_context = waveform_context;
            self.lib.embedding_context = embedding_context;

            Beatrice20rc0_DestroyPhoneContext1(old_phone_ctx);
            Beatrice20rc0_DestroyPitchContext1(old_pitch_ctx);
            Beatrice20rc0_DestroyWaveformContext1(old_waveform_ctx);
            Beatrice20rc0_DestroyEmbeddingContext(old_embedding_ctx);
        }

        self.set_target_speaker(self.info.target_speaker as u32)?;
        while self.set_key_value_speaker_embedding() {}

        self.set_min_source_pitch(self.info.min_source_pitch);
        self.set_max_source_pitch(self.info.max_source_pitch);
        self.set_vq_num_neighbors(self.info.vq_num_neighbors);
    ...
    pub fn set_target_speaker(&mut self, speaker: u32) -> Result<(), BeatriceError> {
        ...
        // assert
        {
            ...
            debug_assert!(
                self.lib.additive_speaker_embeddings.len()
                    == n_speakers_plus1 * BEATRICE_WAVEFORM_GENERATOR_HIDDEN_CHANNELS
            );
        ...
        unsafe {
            ...
            {
                let offset =
                    new_target_speaker_id as usize * BEATRICE_WAVEFORM_GENERATOR_HIDDEN_CHANNELS;
    ...
    fn infer_slice(
        ...
        // ExtractPhone
        let mut phone = [0.0_f32; BEATRICE_20RC0_PHONE_CHANNELS];
        unsafe {
